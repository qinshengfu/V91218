...
<body class="no-skin">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center">XMC钱包</th>
									<th class="center">USDT钱包</th>
									<th class="center">USDT累积</th>
									<th class="center">对冲钱包之USDT</th>
									<th class="center">对冲钱包之XMC</th>
									<th class="center">推荐人数</th>
									<th class="center">推荐人</th>
									<th class="center">会员等级</th>
									<th class="center">团队人数</th>
									<th class="center">团队业绩</th>
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>${var.XMC_WALLET}</td>
											<td class='center'>${var.USDT_WALLET}</td>
											<td class='center'>${var.USDT_COUNT}</td>
											<td class='center'>${var.HEDGING_USDT}</td>
											<td class='center'>${var.HEDGING_XMC}</td>
											<td class='center'>${var.RECOMMENDED_NUMBER}</td>
											<td class='center'>${var.RECOMMENDER}</td>
											<td class='center'>${var.USER_RANK}</td>
											<td class='center'>${var.TEAM_AMOUNT}</td>
											<td class='center'>${var.TEAM_ACHIEVE}</td>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<a class="btn btn-xs btn-danger" title="设为M8" onclick="appoint('${var.ACCOUNT_ID}', '${var.USER_NAME}', '${var.USER_RANK}');">
														设为M8
													</a>
													</c:if>
												</div>
	...
	<script type="text/javascript">
		...
		// 指派M8
		function appoint(Id, name, rank){
			bootbox.confirm("确定要将"+ name +"设置为M8吗?", function(result) {
				if(result) {
					top.jzts();
					if (rank == 9) {
						alert("此用户也是M8的级别")
					...
					var url = "<%=basePath%>front/appoint.do?ACCOUNT_ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>account/goEdit1.do?ACCOUNT_ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
