...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="lncome_details/outList.do" method="post" name="Form" id="Form">
										...
										<tr>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.edit == 1 }">
														<a class="btn btn-xs btn-success" onclick="adopt('${var.LNCOME_DETAILS_ID}');">
															通过
														</a>
														<a class="btn btn-xs btn-danger" onclick="reject('${var.LNCOME_DETAILS_ID}');">
															驳回
														</a>
													</c:if>
												</div>
	...
	<script type="text/javascript">
		...
		// 通过申请
		function adopt(Id){
			bootbox.confirm("确定要通过申请吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>front/adopt.do?LNCOME_DETAILS_ID="+Id+"&tm="+new Date().getTime();
		...
		function reject(Id){
			bootbox.confirm("确定要驳回吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>front/reject.do?LNCOME_DETAILS_ID="+Id+"&tm="+new Date().getTime();
					$.get(url,function(data){
						if (data === "success"){
							alert("成功")
						}
